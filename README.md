...
Turn every YouTube video into a language-learning resource. YouTube Lingo brings transcripts, bilingual translation (Chinese and French), AI overviews, explanations, a vocabulary notebook, and export to Anki, CSV, or Markdown — all in one Chrome side panel, so you can study ideas and language without losing your place.

> This is a fork of [zarazhangrui/youtube-digest](https://github.com/zarazhangrui/youtube-digest) by Zara Zhang. All core functionality — transcripts, translation infrastructure, AI overviews, and notes — is her original work. This fork adds French/English translation, a vocabulary notebook, and export functionality on top of her foundation.

- Turn captions into a readable, searchable learning resource.
- Learn languages with the original transcript, a Chinese translation, an aligned Chinese bilingual view, or an aligned French/English bilingual view.
- Build understanding with an AI overview, chapters, key quotes, and selected-text explanations.
- Save words and phrases to a vocabulary notebook with AI-generated, in-context definitions.
...
- Export vocabulary and notes to CSV, Markdown, or Anki flashcards.
- Keep control of your data with your own API keys, local Chrome storage, and no analytics or telemetry.

YouTube Lingo is a bring-your-own-key project installed locally from GitHub. It is not available through the Chrome Web Store, does not include API credits, and does not run a developer-operated server.

![YouTube Lingo demo](YouTube%20Lingo%20demo.png)
...
## Credit and what's changed

**Original work by Zara Zhang** ([zarazhangrui/youtube-digest](https://github.com/zarazhangrui/youtube-digest)): transcript extraction, the Original/中文/双语 transcript views, AI overviews and chapters, selected-text explanations, the Notes feature, and the entire bring-your-own-key architecture. This fork would not exist without her work — if you're looking for the original, unmodified project, start there.

**Added in this fork:**
- Bilingual FR/EN transcript view
- Vocabulary notebook with AI-generated, in-context definitions
- Export to CSV, Markdown, and Anki

This project is MIT licensed, same as the original — see [LICENSE](LICENSE).

## Install with your coding agent

You do not need to understand the code or use the command line. Send this message to your coding agent:

> Download or clone this project into a permanent folder I choose, tell me its exact full path, and use that same folder for Chrome's Load unpacked step. If I need a suggestion during this first installation, offer `~/Documents/youtube-lingo` on macOS or Linux, or `%USERPROFILE%\Documents\youtube-lingo` on Windows, but do not assume either path. Walk me through installation and setup in simple terms. https://github.com/zarazhangrui/youtube-digest

Your agent should:

1. Ask where you want to keep the project, download or clone it there, and tell you the exact full path. If you want a suggestion, it can offer `~/Documents/youtube-lingo` on macOS or Linux, or `%USERPROFILE%\Documents\youtube-lingo` on Windows.
...
Never paste an API key into an AI chat, source file, screenshot, or public message. Enter keys yourself, directly in the YouTube Lingo Settings page. Your coding agent can point to the correct field without seeing the key.
...
3. Choose a permanent folder and unzip the project there. Optional suggestions are `~/Documents/youtube-lingo` on macOS or Linux, or `%USERPROFILE%\Documents\youtube-lingo` on Windows. You may use a different folder.
4. In Chrome, open `chrome://extensions`.
5. Turn on **Developer mode**.
6. Click **Load unpacked**.
7. Select the exact project folder you chose, which must contain `manifest.json`.
8. Pin YouTube Lingo from Chrome's Extensions menu if you want quick access.

Because this is an unpacked extension, it does not update automatically. After downloading an update or changing local files, click **Reload** on the YouTube Lingo card at `chrome://extensions`, then refresh open YouTube tabs. Moving or deleting the source folder breaks the unpacked extension until you load it again from the new location.

## Set up your API keys

YouTube Lingo needs two keys under your own provider accounts:
...
5. Copy the key and paste it into **Supadata API key** in YouTube Lingo Settings.
...
3. Choose **Create new API key**, give it a recognizable name such as `YouTube Lingo`, and create it.
4. Copy the key immediately. The full key may only be shown once.
5. Paste it into **DeepSeek API key** in YouTube Lingo Settings.
...
Open **Settings** from the side panel. You can also open the YouTube Lingo **Options** page from its card at `chrome://extensions` or by right-clicking its toolbar icon. Paste keys only into these Settings fields. Never paste a key into an AI chat, repository file, screenshot, or public message.
...
2. Click the YouTube Lingo extension icon to open the side panel.
...
- Native subtitle tracks returned by Supadata. YouTube Lingo prefers English when available, but may show another native language.
...
YouTube Lingo forces Supadata's `mode=native`. It does not request AI-generated transcripts or perform local audio transcription when native captions are unavailable.
...
- A generated transcript costs **2 credits per video minute**. YouTube Lingo does not use this path because it forces `mode=native`.
...
DeepSeek usage is separate from Supadata. YouTube Lingo does not collect payments or resell access. Set spending limits and monitor both accounts.
...
YouTube Lingo uses plain HTML, CSS, and JavaScript with no build step, so it is a friendly starting point for agent-assisted projects. Ideas to try:

- ~~Add more translation languages and let each person choose a learning language.~~ Done — French/English added.
- Create customized summary templates for lectures, interviews, tutorials, reviews, or research talks.
- ~~Build a vocabulary notebook that saves a word, its sentence, meaning, and video timestamp.~~ Done — see the Vocabulary tab.
- ~~Export notes and vocabulary to Markdown, CSV, Anki, or another study tool.~~ Done — see the Export buttons.
...
There is no YouTube Lingo account system, advertising, analytics, or telemetry. Supadata and DeepSeek still receive data under their own terms and privacy policies. See [PRIVACY.md](PRIVACY.md) for details.

## Troubleshooting

### The Digest button is missing on a YouTube video

- At `chrome://extensions`, find YouTube Lingo and click **Reload**, then refresh the YouTube tab.
...
- At `chrome://extensions`, confirm YouTube Lingo is enabled and click **Reload**.
...
YouTube Lingo will not fall back to generated transcription.
