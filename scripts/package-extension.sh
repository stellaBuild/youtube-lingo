...
mkdir -p "$dist_dir"
temporary_dir="$(mktemp -d "$dist_dir/.youtube-lingo-package.XXXXXX")"
temporary_zip="$temporary_dir/youtube-lingo.zip"
output_zip="$dist_dir/youtube-lingo-v$version.zip"
